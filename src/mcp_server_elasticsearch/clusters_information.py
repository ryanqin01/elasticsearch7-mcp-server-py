...
class ClusterInfo(BaseModel):
    """Model representing elasticSearch cluster configuration."""
...
def load_clusters_from_yaml(file_path: str) -> None:
    ...
    try:
        # Try to elastic and read the file with proper error handling
        try:
        ...
        for cluster_name, cluster_config in clusters.items():
            try:
                ...
                if 'elasticsearch_url' not in cluster_config:
                    result['errors'].append(f'Missing elasticsearch_url for cluster: {cluster_name}')
                    continue
                cluster_info = ClusterInfo(
                    elasticsearch_url=cluster_config['elasticsearch_url'],
                    elasticsearch_username=cluster_config.get('elasticsearch_username', None),
                    elasticsearch_password=cluster_config.get('elasticsearch_password', None),
...
def check_cluster_connection(cluster_info: ClusterInfo) -> tuple[bool, str]:
    ...
    try:
        ...
        from es_client.client import initialize_client_with_cluster
